...
class TrailsControllerTest < ActionController::TestCase
  ...
  test "should create trail" do
    assert_difference('Trail.count') do
      post :create, trail: { can_ride: @trail.can_ride, length: @trail.length, location: @trail.location, name: @trail.name, status: @trail.status }
    end
  ...
  test "should update trail" do
    patch :update, id: @trail, trail: { can_ride: @trail.can_ride, length: @trail.length, location: @trail.location, name: @trail.name, status: @trail.status }
    assert_redirected_to trail_path(assigns(:trail))
  end
